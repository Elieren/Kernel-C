...
/* Начальный LBA раздела по умолчанию для fs_format_mbr() — 2048 секторов. */
#define FS_DEFAULT_PARTITION_LBA 2048u

/* Инициализация файловой системы на IDE диске.
   Автоматически распознаёт оба варианта разметки:
     - superfloppy: FAT16 занимает весь диск с LBA 0 (без таблицы разделов);
     - MBR: сектор 0 — таблица разделов, используется первый найденный раздел FAT16 (тип 0x04/0x06/0x0E). */
int fs_init(ide_disk_t *disk);

/* Форматирование диска без таблицы разделов (superfloppy) — FAT16 создаётся с LBA 0 на весь диск. */
int fs_format(ide_disk_t *disk);

/* Форматирование диска с таблицей разделов MBR: пишет MBR с одним разделом FAT16,
   начинающимся с partition_start_lba (в секторах от начала диска), и форматирует
   этот раздел как FAT16. */
