...
{
    uint8_t ret;
    __asm__ volatile("inb %1, %0" : "=a"(ret) : "Nd"((uint16_t)port));
    return ret;
}

static inline void io_write8(uintptr_t port, uint8_t data)
{
    __asm__ volatile("outb %0, %1" : : "a"(data), "Nd"((uint16_t)port));
}

static inline uint16_t io_read16(uintptr_t port)
{
    uint16_t ret;
    __asm__ volatile("inw %1, %0" : "=a"(ret) : "Nd"((uint16_t)port));
    return ret;
}

static inline void io_write16(uintptr_t port, uint16_t data)
{
    __asm__ volatile("outw %0, %1" : : "a"(data), "Nd"((uint16_t)port));
}

static inline uint32_t io_read32(uintptr_t port)
{
    uint32_t ret;
    __asm__ volatile("inl %1, %0" : "=a"(ret) : "Nd"((uint16_t)port));
    return ret;
}
